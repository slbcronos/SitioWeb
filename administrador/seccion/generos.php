...
switch ($accion) {
    case "Agregar":
        $sentenciaSQL = $conexion->prepare("INSERT INTO genero (nombregenero) VALUES (:nombregenero)");//cambia segun la bd
        $sentenciaSQL->bindParam(':nombregenero', $txtNombre);
    ...
    case "Modificar":
        ...
        $sentenciaSQL = $conexion->prepare("UPDATE genero SET nombregenero=:nombregenero  WHERE id=:id"); // cambia segun los camops de la bd
        $sentenciaSQL->bindParam(':nombregenero', $txtNombre);
    ...
    case "Seleccionar":
        ...
        $sentenciaSQL = $conexion->prepare("SELECT * FROM genero WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $editorial = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
        $txtNombre = $editorial['nombregenero'];
        break;

    case "Eliminar":
        //echo "Precionado Boton Borrar";
        $sentenciaSQL = $conexion->prepare("DELETE FROM genero WHERE id = :id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        echo ("<meta http-equiv='refresh' content='1'>"); //Refresh by HTTP 'meta'
        break;
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM genero ORDER BY nombregenero");
$sentenciaSQL->execute();
            ...
            <form action="" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" required class="form-control" name="txtNombre" id="txtNombre"
                        value="<?php echo $txtNombre; ?>" placeholder="Nombre del Genero">
                </div>
...
<div class="col-md-7">
    <div class="card">
        <div class="card-header">Tabla de Generos</div>
        <div class="card-body">
                    ...
                    <tbody>
                        <?php foreach ($listaAutores as $autores) { ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo $autores['nombregenero'] ?></td>
                                <td>
